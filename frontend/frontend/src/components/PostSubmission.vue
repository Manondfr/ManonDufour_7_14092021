<template>
    <div class="submissionBox">
        ...
        <form aria-label="Formulaire de création de publication" action="#" method="POST">
...
export default {
    ...
    methods:    {
    // MISE A JOUR DU STORE AVEC IMAGE DE LA PUBLICATION A CREER
    onFileSelected() {
        store.dispatch('changeSelectedFile', event.target.files[0]);
    ...
    // MISE A JOUR DU STORE AVEC CONTENU DE LA PUBLICATION A CREER
    onContentChange() {
       store.dispatch('changeContent', event.target.value);
    },
